# BugStopper Local Launcher
# Calls bugstopper_launcher.py (same mintty invocation as work_launcher.py)
# then starts the time-based monitor that auto-closes the window at 1:30 AM.
#
# Usage: Right-click > Run with PowerShell
#   or:  powershell -ExecutionPolicy Bypass -File launch_bugstopper_local.ps1
...
Write-Host "BugStopper Local - Starting..." -ForegroundColor Cyan
Write-Host "Time window: 12:01 AM - 1:30 AM" -ForegroundColor DarkGray
Write-Host ""
...
$minttyProc = Get-Process mintty -ErrorAction SilentlyContinue |
    Where-Object { $_.MainWindowTitle -match "BugStopper Local" } |
...
if ($minttyProc) {
    ...
    Start-Process powershell -ArgumentList @(
        ...
        "-File", "$projectDir\monitor_bugstopper_local.ps1",
        "-ClaudePID", $minttyProc.Id
    ) -WindowStyle Hidden

    Write-Host "Time monitor started in background." -ForegroundColor Green
    Write-Host "BugStopper Local will run until ~1:30 AM `(+ 3 min grace`)." -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Log: $projectDir\bugstopper_local.log"
} else {
