...
APP_ALL_VERSIONS = [
  ...
  },
  {
    version: "1.3.98",
    changes: [
      "Bug Fix: Dashboard Accounts card now includes financing instruments (loans, mortgages, etc.) in the Loans / Financing section. Previously compute_accounts_overview() only iterated Account records, so financing instruments with include_in_net_worth were missing from the Accounts card despite appearing in Net Worth."
    ]
  },
  {
    version: "1.3.97",
  ...
  },
  {
    version: "1.3.96",
    changes: [
      "Bug Fix: ImportExecutorService (Smart Import) now creates canonical transactions and TransactionMigrationMap entries alongside legacy records. Previously imported transactions only existed in legacy tables, causing them to be invisible to dashboard snapshots, month close totals, and soft close summaries that read from the canonical transaction ledger.",
      "Bug Fix: Moved CREDIT account guard check in import deposits to before record creation, preventing unnecessary rollback.",
      "Data Fix: Reset 4 stale reconciliation records (User 6) from 'reconciled' to 'open' after v1.3.95 balance corrections made their stored outside_balance values no longer match computed budget balances."
    ]
  },
  {
    version: "1.3.95",
  ...
  },
  {
    version: "1.3.94",
    changes: [
      "Hotfix: Remove orphaned admin populate endpoint reference from Net Worth dashboard card that caused 500 error after v1.3.93 route removal.",
      "Cleanup: Remove unused net_worth_populate_controller.js and its Stimulus registration."
    ]
  },
  {
    version: "1.3.93",
  ...
  },
  {
    version: "1.3.84",
    changes: [
      "Dashboard Grid Fix: Repaired 3×2 grid layout — 2 extra closing </div> tags in Income & Spending card partial caused slots 5-6 (Recent Payments, Buckets) to render outside the CSS grid container.",
      "Dashboard Grid Fix: All 6 cards now render at equal height in a viewport-locked 3×2 grid with no vertical page scroll.",
    ]
  },
  {
    version: "1.3.83",
  ...
  },
  {
    version: "1.3.81",
    changes: [
      "Local Time: App timezone changed from UTC to Mountain Time — all date logic now uses local time.",
      "Local Time: Replaced all Date.today with Date.current across controllers, models, views, and services.",
      "Local Time: Header date, dashboard cards, recurring generation, soft close, and net worth snapshots all use timezone-aware dates.",
      "Local Time: Fixed raw SQL NOW() in admin DBU controller to use Rails Time.current."
    ]
  },
  {
    version: "1.3.80",
    changes: [
      "Net Worth Snapshots: Skip months before user account creation date to avoid stale historical data."
    ]
  },
  {
    version: "1.3.79",
