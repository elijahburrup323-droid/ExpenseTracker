  ...
  renderRegular(d) {
    ...
    const rows = sorted.map(c => {
      ...
      return `<tr class="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
        ...
        <td class="px-6 py-3 text-sm font-semibold text-right ${amountClass(c.amount)}">${fmt(c.amount)}</td>
        <td class="px-6 py-3 text-sm text-right text-gray-700 dark:text-gray-300">${c.pct.toFixed(1)}%</td>
        <td class="px-6 py-3 text-sm text-right text-gray-500 dark:text-gray-400">${c.count}</td>
      </tr>`
    }).join("")
