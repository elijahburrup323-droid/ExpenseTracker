...
export default class extends Controller {
  ...
  _injectSoftCloseBanner() {
    const messages = {
      deposits_complete: "Some deposits are missing required information. Fill in the highlighted fields so totals calculate correctly."
    }
  ...
  _autoOpenSoftCloseFix() {
    ...
    const entry = this.entries.find(e => e.id === firstId)
    if (entry) {
  ...
  _renderDisplayRow(entry) {
    ...
    const isFixTarget = this._scFixIds && this._scFixIds.has(entry.id)
    ...
    return `<tr class="${rowClass} transition-colors ${pendingClass}">
