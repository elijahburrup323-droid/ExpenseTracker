...
export default class extends Controller {
  ...
  _renderTagFilter() {
    this.tagFilterContainerTarget.innerHTML = renderTagFilterCheckboxes(
      this._allTags, this._selectedTagIds, "spending-by-type"
    )
  }
  ...
  renderRegular(d) {
    ...
    this.summaryHeadTarget.innerHTML = `<tr>
      ${sortTh("Spending Type", "name", this._sort, "spending-by-type", "left")}
      ${sortTh("Amount", "amount", this._sort, "spending-by-type", "right")}
      ${sortTh("% of Total", "pct", this._sort, "spending-by-type", "right")}
      ${sortTh("# Trans", "count", this._sort, "spending-by-type", "right")}
    </tr>`

    if (d.types.length === 0) {
      this.summaryBodyTarget.innerHTML = `<tr><td colspan="4" class="px-6 py-8 text-center text-sm text-gray-400 dark:text-gray-500">No spending data for this month.</td></tr>`
      return
    }

    const sorted = sortData(d.types, this._sort.field, this._sort.dir)
    const rows = sorted.map(t => {
      const icon = t.icon_key ? renderIconSvg(t.icon_key, t.color_key || "blue", "h-5 w-5") : ""
  ...
  renderComparison(d) {
    ...
    let headCols = `<th class="${thClass} text-left">Spending Type</th>
    ...
    // Build unified type list
    const currMap = {}
    d.types.forEach(t => { currMap[t.name] = t })
    const prevMap = {}
    if (hasPrev) d.prev.types.forEach(t => { prevMap[t.name] = t })
    const ytdMap = {}
    if (hasYtd) d.ytd.types.forEach(t => { ytdMap[t.name] = t })
    const varMap = {}
    if (d.variance) d.variance.types.forEach(t => { varMap[t.name] = t })
  ...
  printReport() {
    ...
    if (this._mode === "regular") {
      ...
      tableRows = sortData(d.types, this._sort.field, this._sort.dir).map(t =>
        `<tr>
      ...
      let headCols = `<th style="text-align:left;">Spending Type</th><th style="text-align:right;">${escapeHtml(d.month_label)}</th>`
...
<head>
  <meta charset="utf-8">
  <title>MyBudgetHQ \u2013 Spending by Type</title>
  <style>
...
<body>
  <div class="header">
    <div class="header-left">
      ...
      <div>
        <div class="brand">MyBudgetHQ <span>Spending by Type</span></div>
      </div>
    </div>
