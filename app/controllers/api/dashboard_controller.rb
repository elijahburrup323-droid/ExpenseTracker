module Api
  class DashboardController < BaseController
    ...
    def card_data
      ...
      avg3 = spending_data&.dig(:planned_spending)
    ...
    def compute_spending_overview(ctx)
      ...
      current_user.payment_recurrings.where(use_flag: true).includes(:frequency_master).each do |pr|
        planned_spending += pr.planned_amount_in_month(ctx[:month_start].year, ctx[:month_start].month)
      end
      ...
      current_user.payment_recurrings.where(use_flag: true, account_id: spendable_ids).each do |pr|
        if pr.next_date && pr.next_date >= ctx[:month_start] && pr.next_date < ctx[:month_end]
          ...
          recurring_bills_items << { name: pr.name, amount: pr.amount.to_f, due_date: pr.next_date.to_s }
        end
      end
      ...
      six_months_ago = (ctx[:month_start] - 6.months)
      historical_data = current_user.payments
        .where(payment_date: six_months_ago...ctx[:month_start])
        .group(:spending_category_id)
        .group(Arel.sql("DATE_TRUNC('month', payment_date)"))
        .sum(:amount)

      # Pivot: { category_id => [monthly_totals] }
      category_months = Hash.new { |h, k| h[k] = [] }
      historical_data.each do |(cat_id, _month), total|
        category_months[cat_id] << total.to_f
      end

      # Current month spending per category (already computed in by_category)
      current_spend_by_cat = by_category.each_with_object({}) { |c, h| h[c[:id]] = c[:amount] }

      # Build variable spending items
      ...
      all_cat_ids.each do |cat_id|
        ...
        # Determine estimate: use limit if set, otherwise historical trimmed average
        if lim && lim.limit_value.to_f > 0
          ...
          next  # No limit and no history — skip
        end

        remaining = [estimate - current_spent, 0].max.round(2)
        variable_spending_items << {
          id: cat_id,
          name: cat_name_map[cat_id] || "Unknown",
          estimate: estimate.round(2),
          spent: current_spent.round(2),
          remaining: remaining,
          source: source
        }
      end
...

      {
        ...
        scheduled_remaining: scheduled_payments.round(2),
    ...
    # Trimmed average: drop highest and lowest if 4+ data points, otherwise simple average
    def trimmed_average(values)
      return 0.0 if values.empty?
      return values.sum / values.size.to_f if values.size < 4
      sorted = values.sort
      trimmed = sorted[1..-2]  # drop lowest and highest
