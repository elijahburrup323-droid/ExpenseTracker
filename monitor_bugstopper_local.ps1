# BugStopper Local Time Monitor
# Terminates Claude Code after the BugStopper Local time window closes (1:30 AM).
#
# Usage: powershell -ExecutionPolicy Bypass -File monitor_bugstopper_local.ps1 -ClaudePID 12345
...
$logFile = "C:\Projects\MyBudgetHQ\Web\bugstopper_local.log"
...
Write-Log "BugStopper Local monitor started. PID=$ClaudePID, Process=$($targetProcess.ProcessName), Title=$windowTitle"
...
function Test-PastEndTime {
    $now = Get-Date
    # BugStopper Local runs 12:01 AM - 1:30 AM
    # Past end = (hour > endHour) OR (hour == endHour AND minute >= endMinute)
    # But if hour >= 6, we're well past the window (handles edge case of late start)
