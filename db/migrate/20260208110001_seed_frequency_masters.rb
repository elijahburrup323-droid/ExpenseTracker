class SeedFrequencyMasters < ActiveRecord::Migration[7.1]
  def up
    # Seed Income Frequency Masters if table is empty
    # This ensures production gets the same seed data that development gets from db:seed
    return if IncomeFrequencyMaster.count > 0
  ...
  def down
    # Only delete if these are the seeded records (don't delete user-created ones)
    IncomeFrequencyMaster.where(id: [1..11, 1001..1029, 2001..2020].flat_map(&:to_a)).delete_all
  end
end
